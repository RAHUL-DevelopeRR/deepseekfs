; ═══════════════════════════════════════════════════════════════
; Neuron v4.3 — Memory OS Edition — Professional Windows Installer
; ═══════════════════════════════════════════════════════════════
; Bundles: Launcher .exe + Source + Python venv + Ollama (optional)
; Target: Any Windows 10/11 x64 machine
;
; NEW in v4.3 - Memory OS Features:
; - Activity logging and tracking
; - Jump back in suggestions
; - You might want to revisit recommendations
; - Memory Lane daily recap panel
; - Continuity streak tracking
...
OutputBaseFilename=NeuronSetup_v4.3_MemoryOS
SetupIconFile=assets\neuron_icon.ico
UninstallDisplayIcon={app}\assets\neuron_icon.ico
UninstallDisplayName={#MyAppName} {#MyAppVersion} - Memory OS Edition
...
VersionInfoDescription={#MyAppName} — AI-Powered Semantic File Intelligence with Memory OS
...
[CustomMessages]
english.WelcomeLabel2=This will install [name/ver] on your computer.%n%nNeuron is an AI-powered semantic file search engine for Windows with Memory OS features. It lets you search by meaning, track your activity, get smart suggestions, and browse with a Windows 11 native interface.%n%nNEW in v4.3 - Memory OS:%n  - Activity logging and jump-back-in suggestions%n  - Smart revisit recommendations%n  - Daily recap with Memory Lane%n  - Continuity streak tracking%n%nMinimum Requirements:%n  - Windows 10/11 (64-bit)%n  - 4 GB RAM (8 GB recommended)%n  - 2 GB free disk space%n%nOptional (for AI Summarization):%n  - Ollama (can be installed during setup)%n  - llama3.2:1b model (~700 MB download)
...
Name: "{group}\Memory Lane"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\assets\neuron_icon.ico"; Comment: "View your daily activity recap"
...
Type: filesandordirs; Name: "{app}\storage\activity.db"
